...
- Tasks:
  - Backend: Implement REST endpoints using UUIDs for student resource paths
    - `POST /students` create (accepts cpf inside JSON body), `GET /students`, `GET /students/:id`, `PUT /students/:id` (accepts cpf inside JSON body), `DELETE /students/:id`
    - `POST /classes`, `GET /classes`, `GET /classes/:id`, `PUT /classes/:id`, `DELETE /classes/:id` and `POST /classes/:id/enroll` to add student by CPF provided in the request body (server will resolve CPF -> student UUID internally).
    - Endpoints must validate and sanitize payloads, return typed JSON, and reject unknown properties to prevent mass-assignment.
...
- Tasks:
  - Add `Evaluation` model tied to Student and Class (structure: id, studentId, classId, goalName, status: GoalStatus, updatedAt). `studentId` is the student's UUID.
  - Backend endpoints for evaluations: `POST /evaluations`, `GET /evaluations?studentId=...&classId=...`, `PUT /evaluations/:id`.
...
- Tasks:
  - Implement a `PendingNotificationRepository` (JSON-backed) that accumulates evaluation-change events (studentId, evaluationId, classId, oldStatus?, newStatus, timestamp) where `studentId` is the UUID.
  ...
  - After successful send per student (identified by UUID), remove sent pending entries atomically.
- Acceptance criteria / tests:
...
- Class: { id: string, topic: string, year: number, semester: number, students: string[] /_ cpfs _/ }
